...
### `ignore.ts` — Pattern Matching **[STUB]**

Uses the `ignore` npm package for gitignore-format pattern matching. Combines `globalIgnore` inline patterns with patterns from `additionalIgnore` files (defaults to `.gitignore`). Applied globally before per-rule inclusions.

### `rule.ts` — Rule Types **[STUB]**

Type definitions for rules. Each rule has: name, description (natural language), inclusions (gitignore patterns), source reference (originating file), and a stable filesystem-safe ID slug.
